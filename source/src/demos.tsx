...
function RunningPlot({cycle, prestretch, pace}: {cycle: number; prestretch: number; pace: number}) {
  ...
  return <svg className="mechanics-chart" viewBox="0 0 310 290" role="img" aria-label="Force sharing across a prescribed stride. Target force equals passive spring force plus active actuator force, including negative actuator force when braking.">
    <text x="22" y="19" className="chart-title">Sharing the leg force</text><rect x={px(STANCE_FRACTION)} y="34" width={238 * (1 - STANCE_FRACTION)} height="144" fill="#eff2f3" rx="2"/>
    ...
    <text x="108" y="195" textAnchor="middle">Stance</text><text x="233" y="195" textAnchor="middle">Flight</text><text x="22" y="218" className="chart-small">Normalized force · active can brake (−)</text>
...
function RunningDemo() {
  const [prestretch, setPrestretch] = useState(.6), [pace, setPace] = useState(1.15), motion = useMotion(2.2 / pace);
  const f = runningForces(motion.cycle, prestretch, pace), p = motion.cycle % 1, flight = (p - STANCE_FRACTION) / (1 - STANCE_FRACTION);
  const ground = 264, hx = 235, legRest = 166;
  const footx = f.stance ? 300 - 135 * f.u : 165 + 135 * (flight * flight * (3 - 2 * flight)), footy = f.stance ? ground : ground - 59 * Math.sin(Math.PI * flight);
  const hipStart = ground - Math.sqrt(legRest ** 2 - 65 ** 2), hipEnd = ground - Math.sqrt(legRest ** 2 - 70 ** 2);
  const hy = f.stance ? ground - Math.sqrt((legRest - f.compression / .23 * 25) ** 2 - (footx - hx) ** 2) : hipEnd + (hipStart - hipEnd) * flight - 12 * Math.sin(Math.PI * flight);
  const legLength = Math.hypot(footx - hx, footy - hy), legAngle = -Math.atan2(footx - hx, footy - hy) * 180 / Math.PI;
  const armSwing = Math.sin(motion.cycle * TAU), groundOffset = (motion.cycle * 135 / STANCE_FRACTION) % 24;
  const otherKnee = {x: hx - 38 - 19 * armSwing, y: hy + 62}, otherFoot = {x: hx - 60 + 35 * armSwing, y: hy + 115 - 21 * Math.cos(motion.cycle * TAU)};
  return <Frame title="A spring-assisted running leg" subtitle="A tunable passive spring and an active actuator work in parallel." status={f.stance ? (f.u < .5 ? 'Stance · loading the leg' : 'Stance · returning spring energy') : 'Flight · no ground reaction'} motion={motion}
    ...
    details={<><p>Fig. 6g and Eq. 5 motivate the parallel architecture: Ftarget = ktarget ΔL = Fspring + Factuator. The green element represents an effective passive leg spring; a tensile ligament would require a transmission to provide this support. The orange actuator supplies the remaining force and can apply a negative force when the passive support exceeds the target.</p><p>This illustration prescribes a stance–flight trajectory, with a stance foot fixed relative to the moving ground. Pace changes the cycle rate and the illustrative target stiffness. Passive force uses F(Δs + c) − F(Δs − c), with normalized compression c ≤ 0.23 and the same illustrative J-curve law as above. It demonstrates force sharing, not a validated running simulation or a prediction of energy savings.</p></>}>
    <svg className="mechanics-scene running-scene" viewBox="0 0 500 290" role="img" aria-label="A running figure with an effective passive spring and an active linear actuator in parallel between hip and stance foot. The mechanism compresses and extends through an animated stance and flight cycle.">
      ...
      <path d={`M${hx} ${hy}Q${otherKnee.x + 10} ${otherKnee.y - 15} ${otherKnee.x} ${otherKnee.y}L${otherFoot.x} ${Math.min(otherFoot.y, ground - 15)}`} stroke="#c2cdd1" strokeWidth="11" fill="none" strokeLinecap="round" strokeLinejoin="round"/><path d={`M${otherFoot.x - 7} ${Math.min(otherFoot.y, ground - 15)}h20`} stroke="#a6b7bd" strokeWidth="8" strokeLinecap="round"/>
      <path d={`M${hx} ${hy - 1}Q${hx + 6} ${hy - 37} ${hx + 20} ${hy - 58}`} stroke="#637d8a" strokeWidth="25" fill="none" strokeLinecap="round"/>
      <path d={`M${hx + 18} ${hy - 53}L${hx - 4 - armSwing * 19} ${hy - 31}L${hx - 23 - armSwing * 22} ${hy - 53}`} stroke="#abbcc4" strokeWidth="9" fill="none" strokeLinecap="round" strokeLinejoin="round"/><path d={`M${hx + 20} ${hy - 51}L${hx + 42 + armSwing * 20} ${hy - 34}L${hx + 63 + armSwing * 16} ${hy - 55}`} stroke="#708e9c" strokeWidth="9" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
      <path d={`M${hx + 22} ${hy - 57}l6 -14`} stroke="#8aa1ad" strokeWidth="10" strokeLinecap="round"/><circle cx={hx + 31} cy={hy - 83} r="15" fill="#8aa1ad"/><path d={`M${hx + 43} ${hy - 85}l5 4-5 2`} fill="#8aa1ad"/>
      <g transform={`translate(${hx} ${hy}) rotate(${legAngle})`}>
